rm(list = ls())
setwd("/Users/bhavin/Documents/Fall 2021 - Semester (M1)/CIS 8695 - Big Data Analytics (Ling Xue)/4.NB, SVM and KNN")
...
pima.linear.tune <- tune.svm(type ~ ., data = train.df,
                        kernel = "linear",
                        cost = c(0.001, 0.01, 0.1, 1, 5, 10, 20, 50, 100))
summary(pima.linear.tune)

best.linear <- pima.linear.tune$best.model
linear.pred <- predict(best.linear, newdata = valid.df)
confusionMatrix(linear.pred, valid.df$type)
...
set.seed(123)
kknn.train <- train.kknn(type ~ ., data = train.df, kmax = 30, 
                        distance = 2, 
