rm(list = ls())
setwd("/Users/bhavin/Documents/MSIS - Big Data Analytics/Fall 2021 - Semester (M1)/CIS 8695 - Big Data Analytics (Ling Xue)/7. Dimension Reduction")
...
nn <- neuralnet(y ~., data = train.df, hidden = c(3,2))
nn.pred <- predict(nn, newdata = valid.df)
confusionMatrix(nn.pred, valid.df$y)
